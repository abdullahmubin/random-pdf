...
export default function ImagesPage(){
  ...
  const handleGenerate = async ()=>{
    ...
    try{
      const blob = await apiClient.imagesToPDFWithProgress(files, (percent, loaded, total)=>{
      ...
      <main className={styles.main}><div className={styles.content}>
        ...
        <section className={styles.uploadSection}>
          ...
          <div style={{marginTop:12}}>
            <button className={styles.generateButton} onClick={handleGenerate} disabled={isGenerating}>{isGenerating? 'Processing...':'📥 Convert Images to PDF'}</button>
          </div>
        </section>
        {isGenerating && (
          <div className={styles.generateOverlay} aria-live="polite">
            <div className={styles.generateCard}>
              ...
              <p>{uploadPhase === 'uploading' ? 'Uploading your files — please wait.' : 'Your images are being converted. This may take a few seconds.'}</p>
              <div style={{marginTop:12}}>
